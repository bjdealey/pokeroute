...
for f in pokemon_moves moves move_damage_classes machines; do
...
with open('data/movedex.csv', 'w') as f:
    f.write('id,name,type_id,power,accuracy,pp,class\n')
    for i in sorted(used, key=int):
        r = mv[i]
        f.write(f"{i},{r['identifier']},{r['type_id']},{r['power'] or ''},"
                f"{r['accuracy'] or ''},{r['pp'] or ''},{dc[r['damage_class_id']]}\n")
